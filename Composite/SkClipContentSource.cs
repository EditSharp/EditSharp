...
namespace EditSharp.Composite
{
    ...
    /// interface's own remarks for why SkFrameCompositor is written against
    ...
    /// </summary>
    internal sealed class SkClipContentSource : IClipContentSource, IDisposable
    {
            ...
            VideoClip clip, VideoSourceNode media, int canvasWidth, int canvasHeight)
        {
            ...
            if (nativeWidth <= 0 || nativeHeight <= 0)
                throw new InvalidOperationException(
                    "No native size registered for a VideoSourceNode — RenderContentPreparation must " +
