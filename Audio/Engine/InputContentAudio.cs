...
{
    /// <summary>
    /// An AudioInputNode as a content stream: prepared in the background as soon
    /// as the clip comes into view, read through one streaming reader, and
    /// re-opened at the new position on a seek. An input that fails reads as
    ...
    /// </summary>
    internal sealed class InputContentAudio(AudioInputNode node, AudioSession session) : IContentAudio
    {
        ...
        public AudioInputNode Node { get; } = node;
        ...
        public bool Ready(bool wait)
        {
            ...
            _failedAt = -1;
            _preparing ??= Task.Run(() => Node.PrepareAsync());
            ...
            else
            {
                Fail(_preparing.Exception?.InnerException as SourceUnavailableException
                    ?? new SourceUnavailableException(SourceUnavailableReason.DecodeError, $"{Node.Description} could not be prepared.", _preparing.Exception?.InnerException));
            }
        ...
        private void Fail(SourceUnavailableException ex)
        {
            ...
            if (session.Report is { } report)
            {
                if (report.Record(Node.Id, Node.Description, ex.Reason, ex.Message, at))
                    EditSharpConfig.Logger.LogWarning($"{Node.Description}: {ex.Message} Rendering silence.");
            }
            else
            {
                EditSharpConfig.Logger.LogWarning($"{Node.Description}: {ex.Message} Playing silence.");
            }
        }
