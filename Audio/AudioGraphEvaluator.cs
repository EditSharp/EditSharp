...
namespace EditSharp.Audio
{
    ...
    /// Graph — the audio-side counterpart to ImageGraphEvaluator,
    /// sharing its exact walking algorithm (GraphTopology.Order) and
    ...
    ///     walker ImageGraphEvaluator uses for MergeNode's
    ...
    /// </summary>
    internal static class AudioGraphEvaluator
    {
...
 
                    default:
                        throw new NotSupportedException(
                            $"AudioGraphEvaluator has no dispatch for {node.GetType().Name}.");
                }
            }
 
