...
namespace EditSharp.Render
{
    ...
    ///   - `Channel.BlendMode` is typed as `ChannelBlendMode` — a superset
    ///     wrapper around SKBlendMode (see ChannelBlendMode.cs) rather than
    ///     SKBlendMode directly, so the four modes with no native Skia
    ///     equivalent have a stable slot to be implemented into later via a
    ///     custom SkSL shader, without another breaking enum swap.
    /// </summary>
    internal static class SkChannelCompositor
    {
        /// <summary>
        ...
        {
            using var paint = new SKPaint { BlendMode = ToNative(blendMode) };
        ...
        /// </summary>
        private static SKBlendMode ToNative(ChannelBlendMode mode)
        {
