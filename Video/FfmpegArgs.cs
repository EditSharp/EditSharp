...
{
    /// <summary>
    /// Small shared helpers used across the assembly pipeline: number
    /// formatting for filter strings, and the global filter-threading
    /// arguments.
    ///
    /// This file used to be much larger (as part of GraphUtilities). The
    /// scale/rotation maths (ComputeScaledSize, ComputeRotatedSize,
    /// EvenAtLeast2), the whole family of supersampled rotation helpers, and
    /// NormalizeTimebase all existed to work around ffmpeg's `rotate` filter
    /// — its binary inside/outside edge test, and its habit of dragging
    /// colour toward the fill at partially transparent pixels. The timeline
    /// pipeline does every transform through a single `perspective` call
    /// instead, so none of that is reachable any more and has been removed
    /// rather than left to rot. ResolveSourceRangeAsync and
