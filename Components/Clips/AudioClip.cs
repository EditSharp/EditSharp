...
namespace EditSharp.Components.Clips
{
    ...
    /// types wired into this class's single Audio-domain EffectGraph — see
    /// the static factory methods below and AudioEffectNodes.cs. Also new
...
 
        private AudioClip(EffectGraph graph) => _graph = graph;
 
        public static AudioClip CreateFromSource(Source source, TimeSpan start, TimeSpan duration) =>
            new(EffectGraph.CreateAudioGraph(new MediaAudioSourceNode { Source = source })) { Start = start, Duration = duration };
 
        public static AudioClip CreateTone(
            ...
            new(EffectGraph.CreateAudioGraph(new ToneGeneratorInputNode
            {
                Waveform = waveform,
...
 
        public static AudioClip CreateTimelineEmbed(TimelineReference reference, TimeSpan start, TimeSpan duration) =>
            new(EffectGraph.CreateAudioGraph(new TimelineAudioInputNode { Reference = reference })) { Start = start, Duration = duration };
 
        /// <summary>Escape hatch for a fully custom graph — see VideoClip.CreateCustom's own remarks.</summary>
        public static AudioClip CreateCustom(EffectGraph graph, TimeSpan start, TimeSpan duration)
        {
            if (graph.Domain != EffectDomain.Audio)
                throw new ArgumentException("AudioClip requires an Audio-domain EffectGraph.", nameof(graph));
 
