...
 
            ...
            {
                TimeSpan reach = TimeSpan.FromTicks(Math.Min(span.Ticks, (next.Start - keyframe.Start).Ticks));
                keyframe.OutHandle = new KeyframeHandle<T>(reach, tangentValue);
            }
 
            if (keyframe.InTangentMode == TangentMode.Auto && hasPrev)
            {
                TimeSpan reach = TimeSpan.FromTicks(Math.Min(span.Ticks, (keyframe.Start - prev.Start).Ticks));
                keyframe.InHandle = new KeyframeHandle<T>(reach, _interpolator.Scale(tangentValue, -1f));
            }
        }
 
        /// <summary>
